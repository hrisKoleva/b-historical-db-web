 ...
 * - DB_SCHEMA_PRIMARY: Main historical data schema (default: M3FDBPRD)
 * - DB_SCHEMA_SECONDARY: Additional data schema (default: M3FDBADD)
...
export const DatabaseConfig = {
   ...
   * - Primary: Main historical data (M3FDBPRD)
   * - Secondary: Additional/external data (M3FDBADD)
   */
  schemas: {
   ...
   * - 74M records in database, must always paginate
   */
  limits: {
 ...
 *                  Can be:
 *                  - Single schema: { SCHEMA: 'M3FDBPRD' }
 *                  - Multiple schemas: { SCHEMA_PRIMARY: 'M3FDBPRD', SCHEMA_SECONDARY: 'M3FDBADD' }
 * 
 ...
 *   SCHEMA_PRIMARY: 'M3FDBPRD',
 *   SCHEMA_SECONDARY: 'M3FDBADD'
 * });
 * ```
 */
...
describe('loadQuery', () => {
  ...
  beforeAll(() => {
    ...
    writeFileSync(
      ...
      'SELECT * FROM {{SCHEMA}}.OCUSMA WHERE OKCUNO = @customerNumber;'
    );
  });
  ...
  it('should load SQL file and replace schema', () => {
    const sql = loadQuery(testFile, { SCHEMA: 'M3FDBPRD' });
    
    expect(sql).toContain('M3FDBPRD');
    expect(sql).not.toContain('{{SCHEMA}}');
  });
  
  it('should cache loaded queries', () => {
    const sql1 = loadQuery(testFile, { SCHEMA: 'M3FDBPRD' });
    const sql2 = loadQuery(testFile, { SCHEMA: 'M3FDBPRD' });
  ...
  it('should support multiple schemas', () => {
    ...
    expect(sql).toContain('M3FDBPRD');
    expect(sql).toContain('M3FDBADD');
    expect(sql).not.toContain('{{');
  });
  
  it('should throw error for missing file', () => {
    expect(() => {
      loadQuery('does-not-exist.sql', { SCHEMA: 'M3FDBPRD' });
  ...
  it('should throw error for unreplaced placeholders', () => {
    ...
    expect(() => {
      loadQuery(testFile, { SCHEMA: 'M3FDBPRD' });
 ...
 * 
 * RETURNS:
 *   -- Primary schema fields (M3FDBPRD)
 ...
 *   country NVARCHAR
 *   
 *   -- Secondary schema fields (M3FDBADD)
 ...
 * 
 * WHY TWO SCHEMAS:
 * - Primary (M3FDBPRD): Core historical customer data
 * - Secondary (M3FDBADD): Additional/external customer data
 ...
 *   - Order aggregations use indexed OACUNO
 * 
 * WHY NO PAGINATION:
...
SELECT 
  ...
  ext.ADDITIONAL_FIELD1 AS additionalField1,
  ext.ADDITIONAL_FIELD2 AS additionalField2,
...
GROUP BY 
  ...
  ext.ADDITIONAL_FIELD1, ext.ADDITIONAL_FIELD2;
```
 ...
 *   customerOrderNumber NVARCHAR - Customer's reference number
...
describe('CustomerQueries', () => {
  ...
  it('should contain expected SQL keywords', () => {
    ...
    expect(CustomerQueries.LIST).toContain('M3FDBPRD'); // Schema substituted
  });
  ...
  it('should have detail query with both schemas', () => {
    ...
    const primaryCount = (detailQuery.match(/M3FDBPRD/g) || []).length;
    const secondaryCount = (detailQuery.match(/M3FDBADD/g) || []).length;
...
```bash
# .env.production
DB_SCHEMA_PRIMARY=M3FDBPRD
DB_SCHEMA_SECONDARY=M3FDBADD
...
**You told us you need this.** Better to build for the real requirement than retrofit later. The utility handles this naturally.
...
**Your intuition was correct.** Inline SQL is an anti-pattern. SQL deserves proper tooling, DBA workflow, and separation from TypeScript.

### Why Pagination Everywhere?

**74M records.** Must never allow fetching all data. Enforce at repository level (closest to data source).

### Why Detailed Comments?

**Context.md rule 22.1:** "Code must be self-explanatory for new team members." Comments explain WHY, not just WHAT.

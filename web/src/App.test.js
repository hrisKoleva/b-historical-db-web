...
import App from './App';
const createResponse = (payload) => Promise.resolve({
    ok: true,
    json: async () => payload
});
...
describe('App', () => {
    ...
    it('loads the default query and renders the customer grid', async () => {
        const mockFetch = vi.fn().mockReturnValueOnce(createResponse({
    ...
    it('displays an error banner when the API call fails', async () => {
        ...
        globalThis.fetch = mockFetch;
        render(_jsx(App, {}));
        await waitFor(() => expect(screen.getByRole('alert')).toHaveTextContent('We could not load customers right now. Please try again.'));
    });
});
